...
当前标签体系约定：中文语料使用 **PKU 标注集**（如 `n/nr/ns/nt/v/vn/a/ad/u/p/c/d/r/m/q/w/x`），英文语料使用 **Penn Treebank 标注集**。
...
### 3. 本地预测
- 输入一句中文或英文。
- 中文 `auto` 模式会自动分词（优先 jieba，缺失时回退内置规则），无需手动空格。
- 返回每个词的原始标签（中文为 PKU 标签，英文为 Penn Treebank 标签）。
- 同时返回标签的中文说明，方便阅读。
...
#### （6）词性中文化
当前版本训练时会保留原始标注集标签：中文按 PKU 标签，英文按 Penn Treebank 标签。
网页会同时显示原始标签（如 `nr`、`VBZ`）及中文说明（如“人名”“第三人称单数现在时动词”）。
...
常见词性示例（按标注集保留，不做归并）：
- 中文（PKU）：`n/nr/ns/nt/v/vn/a/ad/u/p/c/d/r/m/q/w/x`
- 英文（PTB）：`NN/NNS/NNP/NNPS/VB/VBD/VBG/VBN/VBP/VBZ/JJ/RB/DT/PRP/IN/CC`
...
### 11. 人民日报语料（你发的这种格式）如何处理

你截图里的格式属于常见人民日报标注风格，形如：

```text
迈向/v 充满/v 希望/n 的/u 新/a 世纪/n ——/w 一九九八年/t 新年/t 讲话/n
[北京/ns 大学/n]nt 是/v 名校/n 。/w
```

本项目现在支持这种格式，转换规则如下：

1. 普通 `词/词性` 直接保留；
2. 形如 `[北京/ns` 的前缀 `[` 会自动去掉；
3. 形如 `大学/n]` 的后缀 `]` 会自动去掉；
4. 形如 `]nt` 的短语闭合标签会跳过（不当作单词）；
5. 最终输出为标准 CoNLL。

也就是说，你可以把人民日报原始文件直接放到 `backend/data/raw/`，再按前面的 `data_converter.py` 命令转换即可。
