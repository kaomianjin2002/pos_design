...
def _fallback_segment_chinese(text: str) -> list[str]:
    """无第三方分词库时的中文回退分词：按标点切段并做双字切分。"""
    tokens: list[str] = []
    buffer = ""
    for ch in text:
        if ch.isspace():
            if buffer:
                tokens.extend([buffer[i:i + 2] for i in range(0, len(buffer), 2)])
                buffer = ""
            continue
...
def segment_chinese_auto(text: str) -> list[str]:
    """中文自动分词：优先 jieba，不可用时回退到内置规则。"""
    if importlib.util.find_spec("jieba") is not None:
        ...
        if tokens:
            return tokens
    return _fallback_segment_chinese(text)
...
def tokenize_text(text: str, tokenizer: str) -> list[str]:
    ...
    if any("\u4e00" <= char <= "\u9fff" for char in stripped) and " " not in stripped:
        return segment_chinese_auto(stripped)
    return stripped.split()
...
def evaluate_model(model: Any, dataset_pairs: list[tuple[list[str], list[str]]]) -> tuple[float, dict[str, int]]:
    gold = [tags for _, tags in dataset_pairs]
    pred = [model.predict(words) for words, _ in dataset_pairs]
    return compute_accuracy(gold, pred), tag_distribution(pred)
...
def train_model(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    valid_accuracy, _ = evaluate_model(model, valid_pairs) if valid_pairs else (0.0, {})
    test_accuracy, distribution = evaluate_model(model, test_pairs) if test_pairs else (0.0, {})
    baseline_accuracy, _ = evaluate_model(baseline, test_pairs) if test_pairs else (0.0, {})
...
def predict_tags(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "model_name": MODEL_NAMES_ZH["structured_perceptron"],
        "tokens": [
            {"word": word, "tag": tag, "tag_name": tag, "tag_desc": tag_to_chinese(tag)}
